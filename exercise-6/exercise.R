...
husky.games.2015$combined.yards <- rushing.yards + passing.yards
# What is the score of the game where the Huskies had the most combined yards?
max.combined <- husky.scores[husky.games.2015$combined.yards == max(husky.games.2015$combined.yards)]
...
MostYardsScore <- function(games) {
  combined.yards <- games$rushing_yards + games$passing.yards
  most.yards <- games[combined.yards == max(combined.yards)]
  print(paste0("On", most.yards$date, "Washington played", most.yards$opponent, ", resulting in ", most.yards$uw_score))
}
